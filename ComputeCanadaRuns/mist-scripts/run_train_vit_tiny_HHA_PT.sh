...
#SBATCH --time=7:58:58
#SBATCH --job-name vit_tiny_HHA_PT
...
cp $SLURM_SUBMIT_DIR/train_vit_tiny_HHA_PT.py $SLURM_TMPDIR
...
python train_vit_tiny_HHA_PT.py $NUM 1
...
if [ "$num" -lt 5 ]; then
      num=$(($num+1))
      ssh -t mist-login01 "cd $SLURM_SUBMIT_DIR; sbatch --export=NUM=$num run_train_vit_tiny_HHA_PT.sh";
fi
