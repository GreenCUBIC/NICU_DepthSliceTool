...
#SBATCH --job-name vit_tiny_synthetic_HHA
...
cp $SLURM_SUBMIT_DIR/train_vit_tiny_synthetic_HHA.py $SLURM_TMPDIR
...
python train_vit_tiny_synthetic_HHA.py $NUM 1
...
if [ "$num" -lt 5 ]; then
      num=$(($num+1))
      ssh -t mist-login01 "cd $SLURM_SUBMIT_DIR; sbatch --export=NUM=$num run_train_vit_tiny_synthetic_HHA.sh";
fi
