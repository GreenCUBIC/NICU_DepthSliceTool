...
#SBATCH --time=23:58:58
#SBATCH --job-name vit_tiny_synthetic_HHA_PT
...
cp $SLURM_SUBMIT_DIR/data/HHA_Depth_PT.zip $SLURM_TMPDIR/data
cd $SLURM_TMPDIR/data
unzip HHA_Depth_PT.zip

# Copy the training script
cp $SLURM_SUBMIT_DIR/train_vit_tiny_synthetic_HHA_PT.py $SLURM_TMPDIR
...
python train_vit_tiny_synthetic_HHA_PT.py $NUM 1

# Copy the output model back to the main slurm folder
cp $SLURM_TMPDIR/savedModels/* $SLURM_SUBMIT_DIR/savedModels/vit_tiny/HHA_PT

num=$NUM
if [ "$num" -lt 5 ]; then
      num=$(($num+1))
      ssh -t mist-login01 "cd $SLURM_SUBMIT_DIR; sbatch --export=NUM=$num run_train_vit_tiny_synthetic_HHA_PT.sh";
fi
