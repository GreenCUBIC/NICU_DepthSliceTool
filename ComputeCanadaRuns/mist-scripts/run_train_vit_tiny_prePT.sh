...
#SBATCH --time=7:58:58
#SBATCH --job-name vit_tiny_prePT
...
cp $SLURM_SUBMIT_DIR/data/DepthFrameFullPrec_prePT.zip $SLURM_TMPDIR/data
cd $SLURM_TMPDIR/data
unzip DepthFrameFullPrec_prePT.zip

# Copy the training script
cp $SLURM_SUBMIT_DIR/train_vit_tiny_prePT.py $SLURM_TMPDIR
...
python train_vit_tiny_prePT.py $NUM 1

# Copy the output model back to the main slurm folder
cp $SLURM_TMPDIR/savedModels/* $SLURM_SUBMIT_DIR/savedModels/vit_tiny/prePT

num=$NUM
if [ "$num" -lt 5 ]; then
      num=$(($num+1))
      ssh -t mist-login01 "cd $SLURM_SUBMIT_DIR; sbatch --export=NUM=$num run_train_vit_tiny_prePT.sh";
fi
